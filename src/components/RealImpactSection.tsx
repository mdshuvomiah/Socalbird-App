...
export function RealImpactSection({ onNavigate }: RealImpactSectionProps) {
  ...
  const __s = (val: any) => typeof val === 'string' ? val : '';
        ...
        <div className="max-w-7xl mx-auto">
                ...
                <div key={index} className="relative group">
                  ...
                  <div className="relative p-8 bg-white/5 border border-white/10 rounded-3xl backdrop-blur-sm hover:border-white/20 transition-all duration-300">
                    <div className="flex flex-col items-center text-center">
                      ...
                      <div className="text-3xl font-bold text-white mb-2">{__s(stat.value)}</div>
                      <div className="text-sm text-gray-400">{__s(stat.label)}</div>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="space-y-12">
            {[content.caseStudy1, content.caseStudy2].filter(Boolean).map((study: any, index: number) => {
              ...
              const Icon = ICON_MAP[study.icon || (index === 0 ? 'Code' : 'MessageCircle')] || Award;
                    ...
                    <div className="relative group">
                      ...
                      <div className="relative rounded-[2.5rem] overflow-hidden border border-white/10 aspect-video bg-white/5 flex items-center justify-center group-hover:border-white/20 transition-all">
                        ...
                        <div className="absolute bottom-6 right-6 p-4 px-6 bg-black/60 backdrop-blur-md rounded-2xl border border-white/10">
                          <div className="text-2xl font-bold text-cyan-400">{__s(study.resultValue || '95%')}</div>
                          <div className="text-xs text-gray-400 uppercase tracking-wider">{__s(study.resultLabel || 'Efficiency Increase')}</div>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 space-y-8">
                    <div className="space-y-4">
                      <div className="inline-flex items-center gap-2 px-3 py-1 bg-cyan-500/10 border border-cyan-500/20 rounded-lg text-cyan-400 text-xs font-bold uppercase tracking-wider">
                        {__s(study.category || (index === 0 ? 'Enterprise Web' : 'SaaS Platform'))}
                      </div>
                      <h3 className="text-3xl md:text-4xl font-bold text-white leading-tight">
                        {__s(study.title || (index === 0 ? 'Scaling a Global E-commerce Powerhouse' : 'Revitalizing Customer Support with AI'))}
                      </h3>
                      <p className="text-gray-400 text-lg leading-relaxed">
                        {__s(study.description || "We partnered with leading innovators to build a high-performance platform that handles millions of monthly active users with sub-second latency.")}
                      </p>
                    </div>

                    <div className="grid grid-cols-2 gap-6 pb-4">
                      {(__s(study.highlights) || (index === 0 ? "Performance,Scale" : "Automation,24/7 Support")).split(',').map((highlight: string, i: number) => (
